...
export const POST: APIRoute = async (context) => {
  try {
    ...
    const body = await parseJsonBody(context.request, createPostSchema);

    const existing = await postsRepo.getPostById(body.id, body.locale);
    if (existing) {
      throw new ApiError(
        `Post with id "${body.id}" and locale "${body.locale}" already exists`,
        409,
      );
    }
