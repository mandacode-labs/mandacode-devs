...
---
Meerkat is an observability platform where an AI Agent directly analyzes infrastructure logs and metrics to detect anomalies. It surpasses the limitations of traditional rule-based alerts by allowing the AI to understand context through natural language queries or events delivered via external webhooks. The AI queries tools like Prometheus and Loki directly to infer causes. Logs are collected using OpenTelemetry and stored in a vector database to support semantic-based searches, and analysis results are delivered to external channels via webhooks.
