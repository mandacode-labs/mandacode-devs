...
description: ルールベースの通知の限界を超え、AIエージェントが直接インフラを分析するMeerkatの設計哲学と実装ストーリー
---
```markdown
## 問題意識
...
ルールベースのアラートの代わりに、AIエージェントが直接ログとメトリクスを読み取り、
PrometheusやLokiなどのツールを呼び出して原因を推論します。
ログはベクトル化して意味検索が可能な形で保存し、
AnalyzerとVectorsという2つのサービスに分けて独立して拡張できるようにしています。
...
OpenTelemetry OTLPで受け取ったログをテンプレート抽出で重複を除去し、
...
これにより、ログの多様性を維持しながらもベクトル化する固有項目の数を大幅に減らすことができます。
...
同じトリガーとクエリに対する重複分析は
5分ウィンドウ内で自動的にブロックされます。

デプロイはHelm Chartで管理し、
ConfigMapには設定とシステムプロンプトを、
SecretにはAPIキーとDBパスワードを分けて保存します。
...
自然言語一言でインフラ状況を把握できる新しいインフラ可視性を提供すること、
