...
预计30分钟后会自动降低。
这样的虚假警报积累起来，会导致在真正有问题时变得麻木。

Meerkat旨在从根本上解决这个问题。
AI Agent直接读取日志和指标，替代基于规则的警报，
并调用Prometheus、Loki等工具进行原因推断。
日志经过向量化后，以便于语义搜索的方式存储，
并分为Analyzer和Vectors两个服务，能够独立扩展。
...
template模式通过Drain算法将日志提取为模板，最大化存储空间和搜索效率。
...
14:23开始，14:45自动恢复”等结论。

工具结果限制在3万字符内，
错误分为查询语法错误、连接失败、查询失败。
LLM会做出“这是我的查询错误，修正后重试”或
...
工作池由1000个缓冲通道和10个工作者组成。
如果队列已满，则立即返回429错误以提供背压。
对同一触发器和查询的重复分析
在5分钟窗口内自动阻止。
...
异步工作池，打造了一个可在实际运营环境中使用的平台。
为遇到规则基础警报瓶颈的团队
提供一句自然语言即可掌握基础设施状况的新型基础设施可视性，
这正是该项目追求的价值。
